...
What it does per article:
  ...
  • If a Pinterest pin image was rendered (pipeline "Pinterest pin" setting ON),
    uploads + attaches it to the post and appends it hidden in the body so a
    Pinterest "Save" browser extension can offer a proper vertical pin
...
# ─────────────────────────────────────────────────────────────
def _upload_media(db, img_path, alt_text, title=None, post_id=None):
    """Upload one image, set its alt text, return (media_id, source_url). Cached.
    Pass post_id to attach the media item to that post (media library "Uploaded to")."""
    ...
    upd_payload = {"alt_text": alt_text or "", "title": title or alt_text or img_path.stem}
    if post_id:
        upd_payload["post"] = post_id
    upd = _request("POST", _api(f"media/{media_id}"), json=upd_payload)
...
def _pin_block(url, alt, description):
    ...
    'Save' browser extension (which reads the DOM, ignoring display:none) has
    a nicely-cropped vertical pin to offer, without it appearing in the post."""
    return (f'<!-- wp:html -->\n'
            f'<img src="{_esc_attr(url)}" alt="{_esc_attr(alt)}" '
            f'class="pinterest-pin-image" loading="lazy" '
            f'data-pin-description="{_esc_attr(description)}" '
...
def phase_publish(structured, rendered, out_dir, url, db, respect_auto=True):
    ...
    # ── 6) Pinterest pin image (optional) ────────────────────────
    # Uploaded + attached to this post, then appended to the body as a hidden
    # image so a Pinterest "Save" browser extension can offer a nice vertical
    # pin instead of grabbing the landscape feature image.
    pin_path = _resolve_img(out_dir, rendered.get("pin"))
    if pin_path:
        pin_id, pin_url = _upload_media(db, pin_path, alt_text=title, title=title, post_id=post_id)
        pin_block = _pin_block(pin_url, title, meta or title)
        r_content = _request("POST", _api(f"posts/{post_id}"),
                             json={"content": content + "\n\n" + pin_block})
        if r_content.status_code >= 400:
            warn(f"Failed to attach Pinterest pin image to post {post_id}: {r_content.text[:150]}")
        else:
            ok(f"Pinterest pin image attached (media id {pin_id})")
...
def _reconstruct_rendered(out_dir):
    ...
    pin = next((p for p in rdir.glob("pin_rendered.*")), None)
    secs = sorted(str(p) for p in rdir.glob("sec_*_rendered.*"))
    return {"feature": str(feature) if feature else None,
