...
class ControlPanel(ctk.CTk):
    ...
    def _build_settings_page(self, page):
        ...
        # Images
        img = self._card(s, "🖼", "Images", "Output format and resolution for generated visuals")
        ...
        self.format_menu.set(FORMAT_LABELS_REV.get(self.cfg.get("image_format", "webp"),
                                                    "WebP (smaller files)"))
        self.format_menu.pack(fill="x", pady=(2, 8))

        ctk.CTkLabel(img, text="Image resolution", anchor="w", text_color=COLORS["text_dim"]).pack(fill="x")

        stored_res = str(self.cfg.get("image_resolution", "hd"))
        ...
        self.resolution_menu = ctk.CTkOptionMenu(img, values=list(RESOLUTION_LABELS.keys()),
                                                  command=self._on_resolution_change)
        self.resolution_menu.set(initial_label)
        self.resolution_menu.pack(fill="x", pady=(2, 8))

        custom_row = ctk.CTkFrame(img, fg_color="transparent")
        custom_row.pack(fill="x", pady=(0, 0))
        ctk.CTkLabel(custom_row, text="Width", text_color=COLORS["text_dim"]).pack(side="left")
        self.e_res_w = ctk.CTkEntry(custom_row, width=80, height=32, placeholder_text="1000",
                                    fg_color=COLORS["bg_alt"], border_color=COLORS["border"])
        self.e_res_w.insert(0, init_w)
        self.e_res_w.pack(side="left", padx=(6, 16))
        ctk.CTkLabel(custom_row, text="Height", text_color=COLORS["text_dim"]).pack(side="left")
        self.e_res_h = ctk.CTkEntry(custom_row, width=80, height=32, placeholder_text="1000",
                                    fg_color=COLORS["bg_alt"], border_color=COLORS["border"])
        self.e_res_h.insert(0, init_h)
        self.e_res_h.pack(side="left", padx=(6, 0))
        ctk.CTkLabel(custom_row, text="px  (used only when \"Custom…\" is selected)",
                     text_color=COLORS["text_mute"], font=ctk.CTkFont(size=10)).pack(side="left", padx=(10, 0))
        self._on_resolution_change(initial_label)
    ...
    def _on_resolution_change(self, choice):
        ...
        self.e_res_w.configure(state=state)
        self.e_res_h.configure(state=state)

    def _resolve_resolution(self):
        """Return the resolution to pass to automation.py: a preset key or WIDTHxHEIGHT."""
        sel = RESOLUTION_LABELS.get(self.resolution_menu.get(), "hd")
        if sel != "custom":
            return sel
        w, h = self.e_res_w.get().strip(), self.e_res_h.get().strip()
        if not (w.isdigit() and h.isdigit()):
            self._log(f"⚠ Invalid custom resolution '{w}x{h}' — falling back to HD.\n")
            return "hd"
        return f"{w}x{h}"
    ...
    def collect_cfg(self):
        return {
            ...
            "image_resolution": self._resolve_resolution(),
        }
