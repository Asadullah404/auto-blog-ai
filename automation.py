...
# When stdout/stderr aren't attached to a real console (e.g. launched by the
# GUI via subprocess.Popen(stdout=PIPE), or redirected to a log file), Python
# on Windows falls back to the legacy ANSI codepage (cp1252) instead of UTF-8,
# and every emoji/box-drawing character this script prints below would raise
# UnicodeEncodeError immediately. Force UTF-8 so it works both interactively
# and when piped.
...
CONFIG = {
    ...
    "resolution":            "hd",     # heading/section images — sd | hd | fhd | 2k | WxH
...
def _resolve_dims(resolution: str, aspect: float):
    """(w,h) for a preset/custom resolution string. `aspect` (h/w) only applies
    to presets — a custom WIDTHxHEIGHT is always used exactly as given."""
...
def _apply_image_settings(image_format: str, resolution: str,
                           feature_resolution: str = None, pin_resolution: str = None):
    """Resolve the --image-format / --resolution(s) CLI choices into CONFIG.
    `resolution` drives the heading/section images; `feature_resolution` and
    `pin_resolution` default to it/CONFIG's default when not given, so callers
    that only care about one setting can omit the rest."""
    ...
    CONFIG["render_w"], CONFIG["render_h"] = _resolve_dims(resolution, 9 / 16)

    feat_res = feature_resolution or resolution
    CONFIG["feature_resolution"] = feat_res
    CONFIG["feature_w"], CONFIG["feature_h"] = _resolve_dims(feat_res, 0.525)

    pin_res = pin_resolution or CONFIG["pin_resolution"]
    CONFIG["pin_resolution"] = pin_res
    CONFIG["pin_w"], CONFIG["pin_h"] = _resolve_dims(pin_res, 1.5)
...
{% if pin_img %}<img src="{{ pin_img }}" alt="{{ data.title }}" class="pinterest-pin-image" data-pin-description="{{ data.meta_description }}" style="display:none">{% endif %}
<footer>Auto Content Pipeline &middot; {{ generated_at }}</footer>
</body>
</html>"""
...
def _crop_resize(src_path, out_path, out_w, out_h):
    """Resize+center-crop an image to out_w×out_h with no text overlay."""
    img = Image.open(src_path).convert("RGB"); w, h = img.size
    scale = max(out_w / w, out_h / h)
    nw, nh = int(w * scale), int(h * scale)
    img = img.resize((nw, nh), Image.LANCZOS)
    left = (nw - out_w) // 2; top = (nh - out_h) // 2
    cropped = img.crop((left, top, left + out_w, top + out_h))
    _save_image(cropped, out_path)
...
def phase_render(structured, raw_paths, out_dir, url, db):
    ...
    if feat_render_done:
        ...
        if not feat_out.exists():
            try:
                if CONFIG["feature_text_overlay"]:
                    title = structured.get("title", "")
                    _render_image(feat_raw, title, feat_out,
                                   CONFIG["feature_w"], CONFIG["feature_h"], font_path)
                else:
                    _crop_resize(feat_raw, feat_out, CONFIG["feature_w"], CONFIG["feature_h"])
                ok(f"Feature rendered: [cyan]{feat_out.name}[/]")
            except Exception as e:
                warn(f"Feature render failed: {e}"); shutil.copy(feat_raw,feat_out)
        else:
    ...
    # ── Pinterest pin image (optional, tall vertical format) ───
    pin_rel = None
    if CONFIG["pinterest_pin"]:
        pin_out = _img_path(rdir, "pin_rendered")
        pin_render_done = (
            db_render_done(db, url, "pin") and
            pin_out.exists() and
            pin_out.stat().st_size > 5000
        )
        if pin_render_done:
            ok("Pin render [dim]resumed from checkpoint[/]")
        else:
            try:
                title = structured.get("title", "")
                ...
                warn(f"Pin render failed: {e}"); shutil.copy(feat_raw,pin_out)
            db_render_save(db, url, "pin", str(pin_out))  # ★ checkpoint
    ...
    with Progress(SpinnerColumn(style="magenta"),TextColumn("[white]{task.description}"),
        ...
        for i,(sec,raw) in enumerate(zip(sections,sec_raws)):
            ...
            try:
                if CONFIG["heading_text_overlay"]:
                    st=_render_image(raw,heading,out,CONFIG["render_w"],CONFIG["render_h"],font_path)
                    rows.append((f"{i:02d}",heading[:33]+("…" if len(heading)>33 else ""),
                                 st["src"],f"[bold]{st['fs']}[/]","[green]✓[/]"))
                else:
                    _crop_resize(raw, out, CONFIG["render_w"], CONFIG["render_h"])
                    rows.append((f"{i:02d}",heading[:33]+("…" if len(heading)>33 else ""),
                                 "—","—","[green]✓[/]"))
    ...
    for r in rows: tbl.add_row(*[str(c) for c in r])
    console.print(tbl); console.print()
    return {"feature": feat_rel, "sections": sec_outs, "pin": pin_rel}
...
def run_one_url(url: str, seo_skill: str, fresh: bool = False, category: str = "") -> str:
    ...
    while True:   # retry loop — exits only when done, user stops, or too many failures
        ...
        try:
            ...
            # Only takes effect if pipeline_config.json/.env has auto_publish
            # (WP_AUTO_PUBLISH) enabled — phase_publish itself no-ops otherwise.
            try:
...
def main():
    ...
    parser.add_argument("--resolution", type=_resolution_arg, default="hd",
                        help="Heading/section image resolution: a preset (sd=854w, hd=1200w, "
                             "fhd=1920w, 2k=2560w) or a custom WIDTHxHEIGHT, e.g. 1000x1000 "
                             "(default: hd)")
    parser.add_argument("--feature-resolution", type=_resolution_arg, default=None,
                        help="Feature image resolution, set independently of --resolution "
                             "(defaults to --resolution's value if omitted)")
    parser.add_argument("--pin-resolution", type=_resolution_arg, default="1000x1500",
                        help="Pinterest pin image resolution (default: 1000x1500)")
    parser.add_argument("--feature-text", action="store_true",
                        help="Paste the post title onto the feature image (off by default)")
    parser.add_argument("--no-heading-text", action="store_true",
                        help="Do NOT paste section headings onto section images "
                             "(on by default)")
